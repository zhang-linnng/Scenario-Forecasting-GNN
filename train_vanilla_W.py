...
def train_vanilla_W_flow(save_folder, data_folder, data_fname):
    print('--------------------')
    ...
    parser.add_argument(
        '--weight', type=int, default=1, help='trade off KL-divergence and Wasserstein distance')
    ...
    def clamp_weight(model):
        ...
        for module in model.modules():
            if isinstance(module, nn.Conv1d) or isinstance(module, nn.Linear) or isinstance(module, nn.BatchNorm1d):
                module.weight.data = torch.clamp(module.weight.data, -1e-2, 1e-2)
                if module.bias is not None:
                    module.bias.data = torch.clamp(module.bias.data, -1e-2, 1e-2)

    discriminator = Discriminator()
    disc_optimizer = optim.Adam(discriminator.parameters(), lr=args.lr, weight_decay=1e-6)
    ...
    def train_discriminator(data, cond_data):
        ...
        for i in range(5):
            ...
            loss = -discriminator(_data).mean() + discriminator(_synth_data.detach()).mean()
            loss.backward()
            disc_optimizer.step()
    ...
    def calculate_W_distance(data, cond_data, discriminator):
        ...
        _data = torch.cat((data,cond_data),1)
        _synth_data = torch.cat((synth_data,cond_data),1)
        W_distance = best_discriminator(_data).mean() - best_discriminator(_synth_data).mean()
