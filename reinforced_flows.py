...
class CouplingLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.cond_scale_net = nn.Sequential(
            nn.Linear(num_cond_inputs, num_hidden), s_act_func(),
            nn.Linear(num_hidden, num_hidden), s_act_func(),
            nn.Linear(num_hidden, num_inputs))


        self.cond_translate_net = nn.Sequential(
            nn.Linear(num_cond_inputs, num_hidden), t_act_func(),
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        ...
        if mode == 'direct':
            ...
            log_cond_s = self.cond_scale_net(cond_inputs).squeeze() * mask
            cond_t = self.cond_translate_net(cond_inputs).squeeze() * mask
            cond_s = torch.exp(log_cond_s)

            u1 = (inputs * s + t) * (1 - mask)
            u2 =  (inputs * cond_s + cond_t) * mask


            return u1 + u2, log_s.sum(-1, keepdim=True) + log_cond_s.sum(-1, keepdim=True)
            ...
            x1 = (inputs - t) * s 
            x1 = x1 * (1 - mask)
            x2 =  (inputs - cond_t) * cond_s 
            x2 = x2 * mask


            return x1 + x2, -log_s.sum(-1, keepdim=True) - log_cond_s.sum(-1, keepdim=True)
